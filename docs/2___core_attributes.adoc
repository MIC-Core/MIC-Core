...
*_Additional explanation and rationale:_* When using models, it is of legal relevance whether one is entitled to use a certain model and, if so, under which conditions. Among other things, but not exclusively, licenses play an important role here. Failure to pass on information about legal restrictions may result in considerable legal uncertainty for the recipient. Therefore, this information must be provided.
