...
The SRMD (Simulation Resource Meta Data) metadata format is one of the formats specified in the SSP Traceability specification.
...
=== Mapping of MIC Core attributes to the SRMD format
...
Note that the abbreviated explanations are not considered a normative part of this specification, and should not be considered to supersede the normative definitions in the MIC Core specification.

[cols="1,^2m,17",align="center",frame="topbot",options="header"]
|===
...
| Release | administrative-data.release | Unique identifier, preferably human-readable (i.e. semantically meaningful), for the release of a particular simulation model.
